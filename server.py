...
# In-memory storage for active MCP file upload sessions
...
# Pure MCP File Streaming Tools - for uploading files without HTTP/curl
@mcp.tool()
async def start_file_upload(
    ...
    """
    Start a file upload session for streaming files via MCP protocol.
    
    This enables uploading files in chunks through pure MCP tools,
    without needing HTTP endpoints or curl commands.
    ...
    Returns:
        Dictionary with upload_id for this session
    
    Example:
        {"status": "success", "upload_id": "abc-123", "message": "Upload session started"}
    """
    try:
        # Generate unique upload ID
        upload_id = str(uuid.uuid4())
        temp_file_path = TEMP_STORAGE_DIR / f"{upload_id}_temp"
        ...
        return {
            ...
            "chunk_size_recommendation": "Use 256KB chunks for optimal performance"
        }
    except Exception as e:
        return {
            "status": "error",
            "message": f"Failed to start upload session: {str(e)}"
        }
...
async def append_file_chunk(
    ...
    """
    Append a base64-encoded chunk of data to an active upload session.
    
    For files under 1MB, use 256KB chunks (about 4 chunks total).
    The data must be base64-encoded before sending.
    
    Args:
        upload_id: ID from start_file_upload
        data_chunk: Base64-encoded chunk of file data
        chunk_index: Optional chunk number for tracking (0-based)
    
    Returns:
        Status of chunk processing with progress info
    
    Example:
        {"status": "success", "chunks_received": 2, "bytes_written": 262144}
    """
    try:
        if upload_id not in active_uploads:
            return {
                "status": "error",
                "message": f"Invalid upload_id: {upload_id}. Start a new upload session first."
            }
        
        upload_info = active_uploads[upload_id]
        
        # Check for abandoned uploads (older than 1 hour)
        if time.time() - upload_info["start_time"] > 3600:
            upload_info["file_handle"].close()
            del active_uploads[upload_id]
            return {
                "status": "error",
                "message": "Upload session expired. Please start a new upload."
            }
        ...
        upload_info["total_bytes"] += len(chunk_bytes)
        
        return {
            "status": "success",
            "chunks_received": upload_info["chunks_received"],
            "bytes_written": len(chunk_bytes),
            "total_bytes": upload_info["total_bytes"],
            "chunk_index": chunk_index
        }
    except Exception as e:
        return {
            "status": "error",
            "message": f"Failed to append chunk: {str(e)}"
        }
...
async def finish_file_upload(
    ...
    """
    Finalize a file upload session and prepare for analysis.
    
    This closes the file, uploads it to Gemini, and returns a document_id
    that can be used with all other SWMS analysis tools.
    
    Args:
        upload_id: ID from start_file_upload
    
    Returns:
        Dictionary with document_id for use with analysis tools
    
    Example:
        {"status": "success", "document_id": "xyz-789", "file_size": 524288}
    """
    try:
        if upload_id not in active_uploads:
            return {
                "status": "error",
                "message": f"Invalid upload_id: {upload_id}"
            }
        ...
        # Generate final document ID
        doc_id = str(uuid.uuid4())
        final_path = TEMP_STORAGE_DIR / f"{doc_id}_{upload_info['file_name']}"
        
        # Move temp file to final location
        upload_info["temp_file"].rename(final_path)
        
        # Upload to Gemini if client is configured
        gemini_file = None
        if client:
            try:
                gemini_file = client.files.upload(file=str(final_path))
            except Exception as e:
                print(f"Warning: Could not upload to Gemini: {e}")
        ...
        uploaded_files[doc_id] = {
            ...
            'file_size': upload_info['total_bytes'],
            'upload_time': time.time(),
            'gemini_file': gemini_file,
            'chunks_received': upload_info['chunks_received'],
            'upload_method': 'mcp_streaming'
        }
        ...
        current_time = time.time()
        expired_uploads = [
            uid for uid, info in active_uploads.items()
            if current_time - info["start_time"] > 3600
        ]
        for uid in expired_uploads:
            if "file_handle" in active_uploads[uid]:
                active_uploads[uid]["file_handle"].close()
            del active_uploads[uid]
        
        return {
            "status": "success",
            "document_id": doc_id,
            "message": f"File '{upload_info['file_name']}' uploaded successfully",
            "file_size": upload_info['total_bytes'],
            "chunks_processed": upload_info['chunks_received'],
            "gemini_uploaded": gemini_file is not None,
            "next_steps": [
                f"Use document_id '{doc_id}' with analyze_swms_compliance",
                "Or any other SWMS analysis tool"
            ]
        }
    except Exception as e:
        # Try to clean up on error
        if upload_id in active_uploads:
            if "file_handle" in active_uploads[upload_id]:
                try:
                    active_uploads[upload_id]["file_handle"].close()
                except:
                    pass
            del active_uploads[upload_id]
